...
import { Button, AIBadge } from "@/components/ui-kit";
...
function ComparePage() {
  ...
  const { compare, setSelected } = usePolicyStore();
    ...
    <div className="min-h-dvh bg-background pb-12">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 page-enter">
        <Section title="Overview" cols={gridCols}>
          <Row label="Coverage" cols={gridCols}>{selected.map((p) => <Cell key={p.id}>{fmtINR(p.coverage)}</Cell>)}</Row>
          <Row label="Premium / yr" cols={gridCols}>{selected.map((p) => <Cell key={p.id}>{fmtINR(p.premium)}</Cell>)}</Row>
          <Row label="Deductible" cols={gridCols}>{selected.map((p) => <Cell key={p.id}>{fmtINR(p.deductible)}</Cell>)}</Row>
          <Row label="Term" cols={gridCols}>{selected.map((p) => <Cell key={p.id}>{p.term}</Cell>)}</Row>
        </Section>
        ...
        <Section title="What's covered" cols={gridCols}>
          {Array.from(new Set(selected.flatMap((p) => p.covered))).map((item) => (
            <Row key={item} label={item} cols={gridCols}>
              {selected.map((p) => (
                <Cell key={p.id}>{p.covered.includes(item) ? <Check size={16} className="text-success" /> : <X size={16} className="text-muted-foreground" />}</Cell>
              ))}
            </Row>
          ))}
        </Section>
        ...
        <Section title="What's not covered" cols={gridCols}>
          {Array.from(new Set(selected.flatMap((p) => p.excluded))).map((item) => (
            <Row key={item} label={item} cols={gridCols}>
              {selected.map((p) => (
                <Cell key={p.id}>{p.excluded.includes(item) ? <X size={16} className="text-danger" /> : <Check size={16} className="text-success" />}</Cell>
              ))}
            </Row>
          ))}
        </Section>
        ...
        <Section title="Claim process" cols={gridCols}>
          <Row label="Settlement" cols={gridCols}>{selected.map((p) => <Cell key={p.id}>{p.settlement}</Cell>)}</Row>
          <Row label="Avg. claim time" cols={gridCols}>{selected.map((p) => <Cell key={p.id}>{p.avgClaimDays} days</Cell>)}</Row>
        </Section>
        ...
        <Section title="Add-ons" cols={gridCols}>
          {Array.from(new Set(selected.flatMap((p) => p.addOns))).map((item) => (
            <Row key={item} label={item} cols={gridCols}>
              {selected.map((p) => (
                <Cell key={p.id}>{p.addOns.includes(item) ? <Check size={16} className="text-success" /> : <X size={16} className="text-muted-foreground" />}</Cell>
              ))}
            </Row>
          ))}
        </Section>

        <div className={`grid gap-3 rounded-2xl border border-primary/20 bg-[#FAFAFE] p-5 ${gridCols} mt-6`}>
          <div className="flex items-center gap-2"><Sparkles size={16} className="text-primary" /><span className="font-semibold text-primary">AI Insight</span><AIBadge /></div>
          {selected.map((p) => (
            <div key={p.id} className="text-sm text-foreground/90">{p.aiInsight}</div>
          ))}
        </div>
        ...
        <div className={`mt-6 grid gap-3 ${gridCols}`}>
          <div />
          {selected.map((p) => (
            <Button key={p.id} className="w-full" onClick={() => pick(p.id)}>Select This Plan</Button>
          ))}
        </div>
      </div>
    </div>
