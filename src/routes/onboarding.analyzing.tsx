  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-background px-4 py-10">
      <div className="relative mb-8">
        ...
        <span className="absolute -inset-2 rounded-full border-2 border-primary/40" style={{ animation: "ping-slow 2s ease-out infinite" }} />
      </div>
      ...
      <ul className="w-full max-w-md space-y-2">
        {STEPS.map((s, i) => {
          ...
          return (
            <li key={s} className={`flex items-center gap-3 rounded-xl border bg-white px-4 py-3 text-sm transition-all ${complete ? "border-emerald-200" : active ? "border-primary" : "border-border"}`}>
              <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${complete ? "bg-emerald-100 text-emerald-700" : active ? "ai-gradient-bg text-white" : "bg-gray-100 text-gray-400"}`}>
