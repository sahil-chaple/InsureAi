...
function PolicyIssuedPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const p = await issuePolicy(selectedPlan.id, user?.name || "Customer");
      setPolicy(p);
      addPolicy(p);
    })();
    confetti({ particleCount: 120, spread: 80, origin: { y: 0.3 }, colors: ["#4F46E5", "#7C3AED", "#059669"] });
    const t = setTimeout(
      () => confetti({ particleCount: 60, spread: 60, origin: { y: 0.4 }, colors: ["#4F46E5", "#7C3AED"] }),
      500,
    );
    return () => clearTimeout(t);
  }, [selectedPlan?.id, user?.name, addPolicy, selectedPlan]);

  if (!selectedPlan) {
    return (
      <div className="flex min-h-dvh items-center justify-center px-4">
        <div className="card-base p-8 text-center">
          <h1 className="mb-3 text-xl font-bold">No plan found</h1>
        ...
        <div className="card-base p-6 text-left">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="font-semibold">{policy ? fmtDate(policy.validFrom) : "—"}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Valid to</div>
              <div className="font-semibold">{policy ? fmtDate(policy.validTo) : "—"}</div>
            </div>
